	...
	export default {
		...
		methods: { 
			...
			getCategory(){
		  	...
		  	onSubmit() { 
		  		var that = this;  
		  		this.req.post(that.API_PROXY + '/admin/add_product',that.forms)
		  		.then((res) =>{  
			...
			editProduct(id){				
				...
				var that = this; 			
				this.req.get(that.API_PROXY + '/product/'+ id)
				.then((res) =>{   
					for(var x in that.forms){
					...
					that.upfile_list = [{url:res.data.imgs.url}]; 
					that.sub_btn=false;
				});  	
			...
			onEdit(){ 		 	
				var that = this;  
		  		this.req.post(that.API_PROXY + '/admin/edit_product',that.forms)
		  		.then((res) =>{  
		  			var res_code=res.status.toString(); //返回结果状态码转字符串取第一位数
					if(res_code.charAt(0) == 2){  
		  				this.$message({
