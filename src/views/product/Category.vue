	...
	<div id="category">
		<transition appear appear-active-class="animated fadeInLeft">
			<div class="category">
				...
				<!-- 弹窗--修改分类 -->
			 	<el-dialog title="" :visible.sync="editbox"  
			 	style='width: 80%'>
					<el-form :model="editform">
						...
						<el-form-item label="上级分类" :label-width="formLabelWidth">
							<el-select v-model="editform.pid" placeholder="请选择分类">											
								...
								<el-option v-for="item in category_top"  
	...
	export default {
		...
		data() {
			return { 
				...
				editindex:0,
				upfile_url:this.API_PROXY + '/admin/upload/img',
				upfile_data:{
					use:'category'
				}, 
				upfile_head:{
		...
		methods:{
			getCategory(){
				...
				var arr=[]; 
		  		//获取所有分类	
		  		this.req.get(that.API_PROXY + '/admin/get_category_all')
		  			.then((res) =>{ 
			...
			del(id,index){
				...
				this.$confirm('是否删除?', '提示', {
		         ...
		         this.req.put(that.API_PROXY + '/admin/del_category',{id:id})
		  			.then((res) =>{    
			...
			onSubmit(index){ 
		        var that = this;  
		        this.req.put(that.API_PROXY + '/admin/update_category',{form:that.editform})
		  			.then((res) =>{ 
			...
			onswitch(id){
				var that = this; 
				this.req.put(that.API_PROXY + '/admin/update',{id:id,db:'category',field:'is_visible'})
		  			.then((res) =>{ 
						console.log(res);
					});  
			},
...
#category{
	.category{line-height: 30px;text-align:left;}
