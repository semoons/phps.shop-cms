			...
			<template v-if="!addShow">
				<div class="list-head">
		            ...
		            <div class="liat-head-02">
		              手机号：<el-input placeholder="请输入内容" style='width: 70%' size='small' v-model="value"></el-input>
		            </div>
				...
				<el-table :data="member" border style="width: 100%">
					...
					<el-table-column prop="is_visible" label="是否禁用" width="80">
...
methods:{
	...
	post_member(index=1,keywords=''){
		var that = this;
		this.req.post(that.API_PROXY + "/admin/get_user_all",{page:index,size:8,keywords:keywords}).then((res) => {
	...
	handleEdit(row){  
	  ...
	  back(){
		...
		onswitch(id){ 
			var that = this; 
			this.req.put(that.API_PROXY + '/admin/update',{id:id,db:"user",field:"is_visible"})
			.then((res) =>{ 
				console.log(res);
			});  
		}  		 
...
.member{line-height: 30px;text-align: left;}
